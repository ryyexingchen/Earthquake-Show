...
- JMA 增量回补：启动和手动刷新按各来源缓存最新 `IssuedAt` 过滤 JMA XML Feed 旧条目，并在来源状态显示 Feed 覆盖范围；Feed 没有历史分页时只提供覆盖诊断
- 下一步：在真实运行中记录增量回补的 Feed 覆盖范围；若离线时间超过上游 Feed 保留窗口，再设计 JMA 历史归档或用户导入 XML 的补回路径
