...
- 地图启动优化：默认加载约 3.2 MB 的区域概览层、约 4.6 MB 的市町村概览层和约 3.35 MB 的区域边界概览层，高精度资源保留为后续按需加载输入；地图使用蓝色海洋和浅暖色陆地。区域面在正式边界存在时不再重复描边，避免简化几何错位；真实 Release 启动性能基准仍待记录。
...
- 下一步：记录真实 Release 启动内存、首次绘制耗时和不同窗口尺寸截图，确认边界缝隙与颜色对比后再恢复其他页面制作
