...
- 当前版本：`0.9.0`
- 已实现：原生主窗口三栏布局、离线日本示意地图、震度区域/震源/观测点图层、虚拟化地震事件列表、搜索、五类筛选、三种排序、刷新、键盘选择和紧凑窗口详情抽屉
...
当前应用已经具备离线地图和完整事件列表交互，但运行时仍接入空内存仓储，固定 JMAXML 尚未解析，因此正常启动时显示空列表。缓存、报文解析和网络数据源仍未实现。当前地图轮廓是随应用打包的非官方示意资源，不用于行政或预报区域判断。
