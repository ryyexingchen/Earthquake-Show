...
- 海啸数据源：`0.45.0` 已新增独立 JMA XML Feed 适配器，筛选 `VTSE41/51/52`、保存原始 XML 并支持长期 Feed 增量回补；尚未接入 SQLite、JSON 补充源或海啸专页
- 下一步：用真实 `VTSE41/51/52` 样例验收预计到达、预计高度和观测字段，再接入 SQLite 和海啸专页
