...
- 数据边界：通过仓储接口读取、查询、订阅和刷新事件；Infrastructure 使用 SQLite 保存报文和来源状态
- 页面状态：保存事件列表、当前事件、报文版本、搜索筛选、排序、地图、来源、加载、离线和错误状态
- 下一步：接入真实 JMA 数据源和刷新机制
...
当前应用启动时优先读取 `%LOCALAPPDATA%\EarthquakeShow\earthquake-cache.db`。首次运行或缓存为空时写入随程序复制的官方 `VXSE51/52/53` 和订正夹具，将它们合并为一个熊本地震事件；缓存损坏或不可用时回退到固定样本并在页面显示缓存状态。时间线可以切换报文快照，原始 XML 以只读方式查看。固定样本是开发期数据，不是实时 Feed；真实网络数据源和清缓存按钮仍未实现。当前地图轮廓是随应用打包的非官方示意资源，不用于行政或预报区域判断。
