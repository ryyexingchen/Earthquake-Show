...
- 数据边界：通过仓储接口读取、查询、订阅和刷新事件；Infrastructure 使用 SQLite 保存报文和来源状态，并通过 JMA JSON 公共端点补充实时摘要
- 页面状态：保存事件列表、当前事件、报文版本、搜索筛选、排序、地图、来源、加载、离线和错误状态
- 下一步：接入 JMA XML 实时报文并完善跨来源优先级
...
- 实时刷新：需要能够访问 `https://www.jma.go.jp/`；不需要 API Key。没有网络时仍可读取本地缓存。
...
当前应用启动时优先读取 `%LOCALAPPDATA%\EarthquakeShow\earthquake-cache.db`。首次运行或缓存为空时写入随程序复制的官方 `VXSE51/52/53` 和订正夹具，页面可用后请求 JMA `list.json`，把实时摘要合并进 SQLite。网络、限流或解析失败不会清空缓存，页面会显示对应来源状态。JMA JSON 目前只提供列表摘要，区域、市町村、观测点和权威报文时间线仍以固定 XML 为主；JMA XML 实时 Feed 尚未接入。当前地图轮廓是随应用打包的非官方示意资源，不用于行政或预报区域判断。
