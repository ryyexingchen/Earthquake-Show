...
- WebSocket 状态：P2PQuake 文本消息流已接入应用生命周期和 SQLite，支持单连接、传输层 keep-alive、主动连接轮换、断线重连、HTTP/WS 状态分离、状态栏显示重连次数、JST 下次重试时间、连接持续时间、最近错误详情、最近消息活性和连接异常统计，以及窗口关闭取消
- 页面状态：保存事件列表、当前事件、报文版本、来源差异、可能关联来源、自动刷新状态、搜索筛选、排序、地图、来源、加载、离线和错误状态
- 下一步：补充 WebSocket 连接策略设置和真实网络长时间验证
