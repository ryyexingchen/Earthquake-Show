...
- 海啸接口调研：JMA 官方 XML 使用 `eqvol.xml`/`eqvol_l.xml` PULL Feed，至少包含 `VTSE41/51/52`；官网 `tsunami/data/list.json` + 详情 JSON 可作为补充，P2PQuake `/v2/jma/tsunami` 仅作为非官方降级源。当前尚未把这些接口接入应用
- 下一步：接入真实 `VTSE*` 海啸数据源并制作海啸专页；历史 Feed 超出保留窗口时再设计归档或用户导入 XML 路径
