...
- 当前版本：`0.10.0`
- 已实现：原生主窗口三栏布局、固定 JMAXML 解析与真实样本接入、离线日本示意地图、震度区域/震源/观测点图层、虚拟化地震事件列表、搜索、五类筛选、三种排序、刷新、键盘选择和紧凑窗口详情抽屉
...
当前应用启动时会解析随程序复制的官方 `VXSE51/52/53` 和订正夹具，将它们合并为一个熊本地震事件，并在列表和地图中显示。它们是开发期固定样本，不是实时 Feed；网络数据源、SQLite 缓存和完整详情仍未实现。当前地图轮廓是随应用打包的非官方示意资源，不用于行政或预报区域判断。
