...
当前应用启动时会解析随程序复制的官方 `VXSE51/52/53` 和订正夹具，将它们合并为一个熊本地震事件，并在列表、地图和右侧详情中显示。时间线可以切换报文快照，原始 XML 以只读方式查看。固定样本是开发期数据，不是实时 Feed；网络数据源和 SQLite 缓存仍未实现。当前地图轮廓是随应用打包的非官方示意资源，不用于行政或预报区域判断。
