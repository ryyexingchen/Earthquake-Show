...
- 当前版本：`0.32.0`
- 已实现：原生主窗口三栏布局、固定 JMAXML 解析与真实样本接入、4,368 条 JMA 震度观测点坐标目录及 JMAXML 坐标补全、统一高饱和度震度配色与观测点对比边框、摘要/区域-市町村-观测点树/报文时间线/原始数据详情、报文快照切换、JMA 离线真实地震区域地图、震度区域/震源/当前报文观测点图层、按震度控制观测点绘制层级、已匹配区域的地图定位、震度速報无点位时按区域自动跟随、虚拟化地震事件列表、搜索、五类筛选、三种排序、刷新、键盘选择和紧凑窗口详情抽屉
- 地图启动优化：默认加载约 3.2 MB 的 JMA 概览层，高精度区域资源保留为后续按需加载输入；地图使用蓝色海洋和浅暖色陆地。
