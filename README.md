...
- 地图启动优化：默认加载约 3.20 MB 的区域概览层、约 4.58 MB 的市町村概览层和约 3.35 MB 的区域边界概览层；缩放超过 `ZoomLevel > 2` 时后台按需切换 `0.003°` 中精度资源，超过 `ZoomLevel > 8` 时再按需切换区域/市町村高精度资源，缩回时恢复对应层级，失败时保留当前地图。地图缩放范围统一限制为 `0.5–24` 倍，精度替换时保持当前画布中心；地图使用蓝色海洋和浅暖色陆地。区域面在正式边界存在时不再重复描边，支持鼠标左键平移和滚轮以指针位置为中心缩放，手动操作会暂停自动跟随；真实 Release 启动性能基准仍待记录。
