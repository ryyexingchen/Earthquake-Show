...
- 地图性能诊断：完整重绘使用后台调度，拖动期间只更新内容位移；高精度几何在拖动期间暂存，松开后按最新视口校验并复用或重新加载；区域、震度区域、市町村、边界和区域高亮合并为单个 `DrawingVisual`，观测点/震源仍按原顺序保留独立元素。`[MapDebug]` 日志输出到 Visual Studio“输出→调试”和启动应用的终端，重点字段为 `RenderComplete` 的 `elapsed/build/children` 以及 LOD 的 `LoadBegin/LoadApplied/LoadReadyDeferred`。
