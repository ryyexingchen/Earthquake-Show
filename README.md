...
- 地图启动优化：默认加载约 3.2 MB 的区域概览层、约 4.6 MB 的市町村概览层和约 3.35 MB 的区域边界概览层，高精度资源保留为后续按需加载输入；地图使用蓝色海洋和浅暖色陆地。区域面在正式边界存在时不再重复描边，避免简化几何错位；地图缩放上限已提高到 12 倍；真实 Release 启动性能基准仍待记录。
...
- 下一步：实现鼠标左键拖动平移和滚轮缩放，并让滚轮以指针位置为缩放中心；随后再做摘要报文导航和时间线宽度统一
