...
- 当前版本：`0.66.44`
- 本版本地图性能修复：高精度跨缓存范围时保留已经平移的视觉层，只在新几何加载完成后重绘一次；临时地图日志改为单通道输出，并降低拖动采样频率。
- 高精度过渡策略：首次从概览进入高精度时仍可使用中精度过渡；已经处于高精度时跨缓存范围不再切换或重绘旧层，只等待新高精度几何替换。
- 本版本地图性能修复：高精度加载判定统一按约 1° 缓存瓦片，同一瓦片内拖动复用现有视觉层；同级别在途请求合并，中精度地图只解析一次，高精度换瓦片期间保留已有视觉层以避免双重重绘。
...
- 地图性能诊断：完整重绘使用后台调度，拖动期间只更新内容位移；同一精度且内容稳定时拖动提交复用现有视觉层，只更新平移后的地理中心；滚轮缩放期间只对现有缓存执行临时缩放，停止滚轮约 120ms 后再一次性提交重绘和 LOD 检查；手动拖动期间自动刷新收到新报文不会触发 `AutoScale` 或清空持久地图中心；高精度几何在拖动期间暂存，松开后按最新视口校验并复用或重新加载；地图只绘制有明确震度且有坐标的观测点；区域、震度区域、市町村、边界和区域高亮合并为单个 `DrawingVisual`，观测点/震源仍按原顺序保留独立元素。`[MapDebug]` 日志输出到 Visual Studio“输出→调试”和启动应用的终端，`RenderComplete` 额外输出 `stages=legend/static/markers` 分阶段耗时，重点字段仍包括 `elapsed/build/children`、`AutoScaleSkipped` 以及 LOD 的 `LoadBegin/LoadApplied/LoadReadyDeferred`。
...
- 观测点绘制优化：高精度下观测点圆圈、震度标签和观测点高亮合并为单个 `DrawingVisual`，震源保持最后绘制；`RenderComplete` 的 `children` 应从每点多个控件降为少量图层宿主。
- 高精度中心保持：几何异步替换在静止状态锁定替换前画布中心，拖动状态保留已提交中心和像素位移；请求开始时的中心只在拖动且尚无已提交中心时兜底使用，避免加载完成后跳回旧位置。拖动期间延迟 LOD 检查，重复检查合并为一个最新请求，松开后再执行。
