...
- 当前版本：`0.66.61`
- 本版本高精度性能优化：边界视口筛选改为单次索引循环和单次不可变数组构建，避免每次重绘创建 LINQ 迭代器与中间集合；边界顺序和筛选结果不变。
- 本版本高精度性能优化：高精度范围缓存改为真正的 LRU，缓存命中会更新访问顺序；容量仍为 3，不增加内存上限，来回拖动时优先复用最近使用的范围。
- 本版本高精度性能优化：边界绘制前使用索引循环检查坐标有效性，避免每条边界先执行 LINQ 全量枚举再重复投影遍历；显示结果和边界简化规则不变。
- 本版本报文切换性能修复：自动缩放导致高精度降级时，抑制旧高精度几何的中间重绘，等待目标 LOD 应用后再绘制；同级或升级仍保留当前精度过渡显示，避免切换期间空白。
- 本版本高精度拖动优化：当静态绘制覆盖范围失效但无需切换 LOD 时，使用 `80ms` 空闲窗口合并补绘；连续拖动只在最后一次统一重建静态层。高精度重载仍等待新几何完成后再绘制，日志输出 `PanRenderDeferred`/`PanRenderResumed`。
- 本版本高精度拖动优化：拖动越过当前高精度覆盖范围时，暂时保留并平移现有视觉层，等待新范围高精度几何完成后再重绘；避免旧高精度图层和新高精度图层连续完整构建造成卡顿。日志输出 `PanRenderDeferred`/`PanRenderResumed` 记录该过程。
- 本版本高精度性能优化：根据当前视野内边界数量自适应线段简化；边界较少时保持 `0.65px`，边界密集时使用 `1px` 屏幕阈值，保留地图覆盖范围和边界首尾点，降低大数据事件的静态几何构建开销；`RenderComplete` 日志输出 `boundarySimplify` 实际阈值。
- 本版本报文切换性能修复：由多个地图属性通知触发的 LOD 检查合并到同一后台调度周期；显式缩放、拖动结束和按钮操作仍立即检查，避免重复计算和重复读取造成切换卡顿。
- 本版本高精度性能优化：边界线按屏幕像素距离进行轻量简化，保留每条边界首尾点，减少报文切换或高精度视图变化时的无效折线投影。
- 本版本地图性能优化：基础区域/市町村/边界几何与选中高亮拆分为独立视觉层，选择变化只重绘高亮层，避免整张高精度静态地图重新投影。
- 本版本 UI 清理：为观测树项显式设置横向拉伸和纵向居中，避免 WPF 默认 `TreeViewItem` 模板在容器初始化期间反复查找未挂接的 `ItemsControl` 绑定源。
- 本版本高亮修复：自动刷新重新生成等价事件快照时不再重建详情树；确需重建时恢复当前都道府县、区域、市町村或观测点选择，并显式处理 TreeView 首次点击切换，避免高亮立即消失。
- 本版本地图性能修复：静态几何缓存对极小的浮点反算误差使用安全容差，避免相同视图因数值抖动重复投影边界；地图状态变化不再重建详情观测树，避免首次选择观测点后高亮被自动清除。
- 本版本地图性能修复：滚轮缩回较低精度时保留当前预览画面，等待目标 LOD 应用后只重绘一次，避免先重建即将被替换的高精度图层；精度升级和同级缩放行为不变。
- 本版本地图完整性修复：高精度静态几何记录最近一次实际绘制范围；累计拖动超出该范围时不再无限复用旧位图，松手后按新中心补绘。绘制缓冲从 15% 增加到 25%，减少拖动中临时露出未绘制区域。
- 本版本地图性能修复：高精度静态几何只投影和绘制当前视野及 15% 缓冲区内的区域、市町村和边界；包围盒在资源解析时一次性计算，`RenderComplete` 同时输出实际绘制数和已加载数。
- 本版本地图性能修复：自动刷新、来源状态、搜索和排序等非地图页面状态不再重建地图图层；一次拖动期间重复的 LOD 延迟检查只记录一条日志。
