...
- 海啸数据源：`0.46.0` 已新增独立 JMA XML Feed 适配器和 SQLite 海啸报文仓储，保存 `VTSE41/51/52` 的完整结构与原始 XML；尚未接入应用自动刷新、JSON 补充源或海啸专页
- 下一步：把 `JmaTsunamiXmlSource` 接入应用启动/刷新生命周期，并持久化来源状态，再制作海啸专页
