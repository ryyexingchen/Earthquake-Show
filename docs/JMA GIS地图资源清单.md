...
- 当前状态：已完成地震细分区域、市町村和都道府县三个 Polygon 图层的离线转换；`0.30.0` 接入地震细分区域概览层，`0.33.0` 接入市町村概览层，`0.33.3` 加载区域边界 LineString 资源，`0.33.4` 完成按报文震度分组，`0.33.5` 接入 WPF 分组绘制，`0.34.0` 完成区域面移除和市町村有效震度填色，`0.34.1` 停止正式边界存在时的区域 Polygon 重复描边并加粗边界线，`0.39.0` 接入概览/中精度按缩放异步切换
...
| `src/EarthquakeShow.App/Assets/Data/Map/jma-earthquake-area-boundaries-medium.geojson` | 1069 | 带相邻区域代码的区域边界中精度 LineString 层 | `0.39.0` 在 `ZoomLevel > 2` 按需加载；0.003 度简化 |
