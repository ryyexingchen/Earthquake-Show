...
- 当前状态：已完成地震细分区域、市町村和都道府县三个 Polygon 图层的离线转换；`0.30.0` 接入地震细分区域概览层，`0.33.0` 接入市町村概览层，`0.33.3` 加载区域边界 LineString 资源，`0.33.4` 完成按报文震度分组，`0.33.5` 接入 WPF 分组绘制，`0.34.0` 完成区域面移除和市町村有效震度填色，`0.34.1` 停止正式边界存在时的区域 Polygon 重复描边并加粗边界线，`0.39.0` 接入概览/中精度按缩放异步切换，`0.59.0` 接入高精度区域/市町村按缩放异步切换，`0.60.0` 接入按视野裁剪和高精度区域轮廓派生
...
| 派生文件 | 特征数 | 用途 | 当前状态 |
| --- | ---: | --- | --- |
| `src/EarthquakeShow.App/Assets/Data/Map/jma-earthquake-areas.geojson` | 194 | 地震信息细分区域高精度层 | `0.62.5` 在 `ZoomLevel > 12` 按 `.index.json` 包络索引只读取当前视野及 20% 缓冲命中的 Feature，越界时先显示中精度并保持持久视口中心 |
...
| `src/EarthquakeShow.App/Assets/Data/Map/jma-earthquake-municipalities.geojson` | 1910 | 地震/海啸市町村高精度层 | `0.62.5` 在 `ZoomLevel > 12` 按 `.index.json` 包络索引只读取当前视野及 20% 缓冲命中的 Feature，越界时先显示中精度并保持持久视口中心 |
...
- 概览层随应用启动加载，当前事件只绘制代码匹配的市町村；全局 `ZoomLevel=1.0` 以全国概览自动适配为基准，中精度三件套在 `ZoomLevel > 2` 后台按需加载并一起替换，区域和市町村高精度层在 `ZoomLevel > 12` 通过 Feature 偏移索引按当前画布包络加 20% 缓冲读取，越界时先显示中精度并异步重载高精度，最多保留当前和上一范围高精度几何，区域轮廓从同一批高精度 Polygon 派生。精度替换前保存当前画布中心，不在启动阶段解析高精度全国运行时对象。
