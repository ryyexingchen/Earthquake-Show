...
- 当前状态：已完成地震细分区域、市町村和都道府县三个 Polygon 图层的离线转换；`0.30.0` 接入地震细分区域概览层，`0.33.0` 接入市町村概览层，`0.33.3` 加载区域边界 LineString 资源，`0.33.4` 完成按报文震度分组，`0.33.5` 接入 WPF 分组绘制但暂未完成区域填充重构
...
| `src/EarthquakeShow.App/Assets/Data/Map/jma-earthquake-area-boundaries-overview.geojson` | 1069 | 带相邻区域代码的区域边界 LineString 候选层 | `0.33.5` App 启动加载、建立索引、按报文震度分组并接入 WPF 绘制；0.015 度简化、0.0002 平方度环过滤 |
...
`0.33.3` 已将候选资源正式复制为 `src/EarthquakeShow.App/Assets/Data/Map/jma-earthquake-area-boundaries-overview.geojson`，共 1,069 条边界 Feature，构建输出由 `Assets\Data\**\*` 规则自动复制。`0.33.4` 在 ViewModel 中按 `areaCode1/areaCode2` 双向索引和当前报文最大有效震度生成 `BoundaryLayers`；`0.33.5` 将每个震度组转换为一个冻结的 WPF `StreamGeometry`，实际绘制仍保留现有区域面填充。
